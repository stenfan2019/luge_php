<div class="layui-form" lay-filter="action_form" id="action_form" style="padding: 20px 30px 0 0;">
    ...
    <div class="layui-form-item more_option">
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" id="controller" value="<?=$item['controller']?>"
                       name="controller"  placeholder="请输入指向控制器" autocomplete="off"
                       class="layui-input">
            </div>
        </div>
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" id="action" value="<?=$item['action']?>" 
                      name="action"  placeholder="请输入指向方法" autocomplete="off" class="layui-input">
            </div>
        </div>
    </div>
    <div class="layui-form-item more_option">
        ...
        <div class="layui-input-inline">
            <input type="text" name="param" value="<?=$item['param']?>" 
                   placeholder="添加路径参数,如有" autocomplete="off" class="layui-input">
        </div>
    </div>
    <?php }else{?>
        <div class="layui-form-item">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" name="icon" value="<?=$item['icon']?>" 
                        placeholder="layui图标代码" autocomplete="off" class="layui-input">
        </div>
...
<script>
    layui.use(['index', 'form', 'admin'], function () {
        ...
        form.on('submit(form_submit)', function (data) {
            ...
            admin.req({
                type: 'post',
                url: "/menu/edit?id=<?=$item['id'] ?>",
                dataType: 'json',
                data: field,
                success: function (res) {
                    if (res.code == '0') {
                        layer.msg('修改成功', {icon: 1}, function () {  //提示完后,才执行里面的
