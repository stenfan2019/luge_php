       ...
       <div class="layui-form layui-card-header layuiadmin-card-header-auto">
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="data_control">
                <a class="layui-btn layui-btn-normal layui-btn-xs" lay-href="/cash/anchor?params={{d.anchor_id}}|uid" lay-text="{{d.nickname}}">
...
<script>
    layui.use(['index', 'table', 'admin'], function () {
        ...
        table.render({
            elem: '#data_list'
            , url: '/withdraw/anchor?data=1'
            , cols: [[
                  {field: 'anchor_id', title: '主播ID',align:'left'}
                , {field: 'username', title: '主播账号',align:'left'}
                , {field: 'nickname', title: '主播昵称',align:'left'}
        ...
        table.on('tool(data_list)', function (obj) {
            ...
            if (obj.event === 'refuse') {
            	layer.confirm('您确定拒绝本次提现申请？', function (index) {
                    admin.req({
                        type: 'post',
                        url: '/withdraw/anchor-refuse?id='+ data.id,
                        dataType: 'json',
                        data: {id: data.id},
            	...
            	layer.confirm('您确定通过本次提现申请？', function (index) {
                    admin.req({
                        type: 'post',
                        url: '/withdraw/anchor-pass?id='+ data.id,
                        dataType: 'json',
                        data: {id: data.id},
                        success: function (res) {
                            if (res.code == '0') {
