...
function __construct($width = 150, $height = 50, $codeNum = 4)
{
    ...
    $this->defaultFont =   __DIR__ . "/../font/simsun.ttc";
}
    //显示验证码图片
    ...
    private function outputText($fontFace=''){
        $fontFace = $fontFace ? $fontFace : $this->defaultFont;

        //画出code
        for($i=0;$i<$this->codeNum;$i++){
            $fontColor=imagecolorallocate($this->image,mt_rand(0,200),mt_rand(0,200),mt_rand(0,200));
    ...
    private function createCode(){
        if(!$this->chinese){
            ...
            $string='0123456789abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ';
        }else{
