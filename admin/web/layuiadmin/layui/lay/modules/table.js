;layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
            ...
            $.ajax({
                ...
                , success: function (res) {
                    if (that.fetchResponse(res, response.statusName) != response.statusCode) {
                        that.renderForm();
                        that.layMain.html('<div class="' + NONE + '">' + (that.fetchResponse(res, response.msgName) || '返回的数据状态异常') + '</div>');
                    } else {
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        //console.log(this.fetchResponse(res, this.config.response.dataName));
        var that = this
            , options = that.config
