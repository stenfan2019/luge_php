...
class FuncHelper
{
     ...
     * @date 2020-12-02
     */
    public static function createRandNum($num=17)
    {
        $str = date('mdHis');
        $end = $num-strlen($str);
        for ($i=0;$i<$end;$i++)
     ...
     * @date 2020-12-03
     */
    public static function generateLotteryResult($period_code)
    {
        if(is_array($period_code) && count($period_code)==3) return $period_code;
        $r = [];
        for ($i=0;$i<3;$i++)
        {
            $r[$i] = substr($period_code,$i,1);
        }
        return $r;
